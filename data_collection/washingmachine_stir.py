...
class WashingMachineEnv():
    ...
    def stir(self,point_cloud,max_value,percentage_below_threshold=None,use_model=False):
        index=0
        continue_stir=True
        while continue_stir:
            ...
            if not use_model:
                ...
                while dis<0.13:
                    ...
                    dis=torch.norm(torch.tensor(point_cloud[place])-torch.tensor(point_cloud[pick]) ).item()
                ...
                place_model_output = self.place_model(pick_pc.transpose(2, 1), place_pc.transpose(2, 1))
            ...
            filename=get_unique_filename(base_filename="data/stir/data",extension=".npz")
            ...
            if place_percentage_below_threshold > 0.05 :
                continue_stir=False
                np.savez(filename,point_cloud=point_cloud,point_cloud_after_stir=point_cloud_after_stir,pick=stir_pick,place=stir_place,flag=1)
                print("stir success")
            elif place_max_value - pick_max_value > 0.1:
                continue_stir=False
                np.savez(filename,point_cloud=point_cloud,point_cloud_after_stir=point_cloud_after_stir,pick=stir_pick,place=stir_place,flag=1)
                print("stir success")
            elif place_percentage_below_threshold - pick_percentage_below_threshold > 0.02:
                continue_stir=False
                np.savez(filename,point_cloud=point_cloud,point_cloud_after_stir=point_cloud_after_stir,pick=stir_pick,place=stir_place,flag=1)
                print("stir success")
            else :
                np.savez(filename,point_cloud=point_cloud,point_cloud_after_stir=point_cloud_after_stir,pick=stir_pick,place=stir_place,flag=0)
                print("stir fail")
