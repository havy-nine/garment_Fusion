...
class BaseEnv:
    ...
    def random_pick_place(self):
        ...
        pick_place_target_positions = copy.deepcopy(self.config.target_positions)

        pick_place_target_positions[2] = place_point

        fetch_result = self.franka.basket_pick_place_procedure(
            pick_place_target_positions, self.attach
        )
    ...
    def get_highest_ratio(self):
        ...
        exec_point = self.point_cloud_camera.get_model_point()
        cprint(f"max_success: {self.point_cloud_camera.max_value}", "cyan")
        with open("Env_Eval/basket_record.txt", "a") as file:
    ...
    def model_pick_whole_procedure(self):
        ...
        while True:
            ...
            if not os.path.exists("Env_Eval/basket_record.txt"):
                with open("Env_Eval/basket_record.txt", "w") as f:
                    f.write("result ")
            else:
                with open("Env_Eval/basket_record.txt", "rb") as file:
                    file.seek(-1, 2)
                    last_char = file.read(1)
                    if last_char == b"\n":
                        with open("Env_Eval/basket_record.txt", "a") as f:
                            f.write("result ")
            ...
            if aff_ratio >= 0.6 or self.stir:
                ...
                target_positions[2] = place_point
