...
from Model.pointnet2_Pick_Model import Pick_Model, Pick_Model_Loss
from Data_Utils.DataLoader_PickModel import DataLoader_PickModel
...
def train(**kwargs):
    ...
    wandb.init(
        ...
        name="WashMachine_Pick_Model",
        config=config,
        resume="WashMachine Pick Model Version 0",
        # mode="online",
    )
    ...
    train_data = DataLoader_PickModel(
        mode="train", data_dir="Data/WashMachine/Stir_Model"
    )
    val_data = DataLoader_PickModel(mode="val", data_dir="Data/WashMachine/Stir_Model")
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.print_freq == 0:
            if not os.path.exists("Model_Train/Model_Checkpoints/WM_Pick_Model"):
                os.makedirs("Model_Train/Model_Checkpoints/WM_Pick_Model")
            torch.save(
                model.state_dict(),
                f"Model_Train/Model_Checkpoints/WM_Pick_Model/WM_Pick_Model_Parameter_{epoch+1}.pth",
            )
