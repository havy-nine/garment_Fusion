...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.print_freq == 0:
            if not os.path.exists("Model_Train/Model_Checkpoints/Basket_Model"):
                os.makedirs("Model_Train/Model_Checkpoints/Basket_Model")
            torch.save(
                model.state_dict(),
                f"Model_Train/Model_Checkpoints/Basket_Model/Basket_Model_Parameter_{epoch+1}.pth",
            )
