...
def train(**kwargs):
    ...
    wandb.init(
        ...
        name="Basket_Pick_Model",
        config=config,
        resume="Basket Pick Model Version 0",
        # mode="offline",
    )
    ...
    train_data = DataLoader_PickModel(mode="train", data_dir="Data/Basket/Stir_Model")
    val_data = DataLoader_PickModel(mode="val", data_dir="Data/Basket/Stir_Model")
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.print_freq == 0:
            if not os.path.exists("Model_Train/Model_Checkpoints/Basket_Pick_Model"):
                os.makedirs("Model_Train/Model_Checkpoints/Basket_Pick_Model")
            torch.save(
                model.state_dict(),
                f"Model_Train/Model_Checkpoints/Basket_Pick_Model/Basket_Pick_Model_Parameter_{epoch+1}.pth",
            )
