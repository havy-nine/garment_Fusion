...
def train(**kwargs):
    ...
    wandb.init(
        ...
        name="Basket_Place_Model",
        config=config,
        resume="Basket Place Model Version 0",
    )
    ...
    train_data = DataLoader_PlaceModel(mode="train", data_dir="Data/Basket/Stir_Random")
    val_data = DataLoader_PlaceModel(mode="val", data_dir="Data/Basket/Stir_Random")
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.print_freq == 0:
            if not os.path.exists("Model_Train/Model_Checkpoints/Basket_Place_Model"):
                os.makedirs("Model_Train/Model_Checkpoints/Basket_Place_Model")
            torch.save(
                model.state_dict(),
                f"Model_Train/Model_Checkpoints/Basket_Place_Model/Basket_Place_Model_Parameter_{epoch+1}.pth",
            )
...
def val(model, val_dataloader):
    ...
    for step, (pick_data, place_data, label) in enumerate(processbar):
        ...
        processbar.set_description(
            "[ VAL PART ], accuracy:%.4f, loss:%4f"
            % (accurate_meter.value()[0], loss_meter.value()[0])
        )
