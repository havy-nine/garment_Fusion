...
class Collision_Group:
    def __init__(self, stage):
        ...
        self.conveyor_belt_group = UsdPhysics.CollisionGroup.Define(
            stage, self.conveyor_belt_group_path
        )
        self.filter_conveyor_belt = self.conveyor_belt_group.CreateFilteredGroupsRel()
        self.collectionAPI_conveyor_belt = Usd.CollectionAPI.Apply(
            self.filter_conveyor_belt.GetPrim(), "colliders"
        )
        ...
        self.special_group = UsdPhysics.CollisionGroup.Define(
            stage, self.special_group_path
        )
        self.filter_special = self.special_group.CreateFilteredGroupsRel()
        self.collectionAPI_special = Usd.CollectionAPI.Apply(
            self.filter_special.GetPrim(), "colliders"
        )
        ...
        self.collectionAPI_conveyor_belt.CreateIncludesRel().AddTarget(
            "/World/Conveyor_belt"
        )
        self.collectionAPI_helper.CreateIncludesRel().AddTarget(
            "/World/Washmachine_Model"
        )
        self.collectionAPI_special.CreateIncludesRel().AddTarget(
            "/World/Washmachine_Model/cube8"
        )
        self.collectionAPI_special.CreateIncludesRel().AddTarget(
            "/World/Washmachine_Model/cube9"
        )
        ...
        self.filter_robot.AddTarget(
            self.garment_group_path
        )  # Franka can't collide with garment
        self.filter_attach.AddTarget(
            self.rigid_group_path
        )  # Attach Object can't collide with wash_machine
        self.filter_attach.AddTarget(
            self.garment_group_path
        )  # Attach Object can't collide with garment
        self.filter_attach.AddTarget(
            self.robot_group_path
        )  # Attach Object can't collide with Franka
        self.filter_conveyor_belt.AddTarget(
            self.rigid_group_path
        )  # conveyor_belt can't collide with wash_machine
        self.filter_conveyor_belt.AddTarget(
            self.robot_group_path
        )  # conveyor_belt can't collide with Franka
        self.filter_conveyor_belt.AddTarget(
            self.attach_group_path
        )  # conveyor_belt can't collide with attach Object
        self.filter_conveyor_belt.AddTarget(
            self.helper_group_path
        )  # conveyor_belt can't collide with washmachine_model
        self.filter_helper.AddTarget(
            self.rigid_group_path
        )  # washmachine_model can't collide with wash_machine
        self.filter_helper.AddTarget(
            self.robot_group_path
        )  # washmachine_model can't collide with Franka
        self.filter_helper.AddTarget(
            self.attach_group_path
        )  # washmachine_model can't collide with attach Object
