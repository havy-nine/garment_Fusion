...
class Point_Cloud_Camera:
    ...
    def initialize(self, garment_num: int):
        ...
        # self.model.load_state_dict(torch.load("Model/Sofa_Model_Finetune_Model.pth"))
        self.model.load_state_dict(
            torch.load("Env_Config/Model/sofa_retrieve_model_finetuned.pth")
        )
        self.model.eval()
        ...
        self.place_model.load_state_dict(
            torch.load("Env_Config/Model/sofa_place_model_finetuned.pth")
        )
        self.place_model.eval()

        self.pick_model = Pick_Model(normal_channel=False).cuda()
        self.pick_model.load_state_dict(
            torch.load("Env_Config/Model/sofa_pick_model_finetuned.pth")
        )
    ...
    def save_point_cloud(self, sample_flag=True, sample_num=4096, save_flag=False):
        # get point_cloud data
