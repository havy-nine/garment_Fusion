...
class WrapFranka:
    def __init__(self, world:World, position=torch.tensor, orientation=[0.0, 0.0, 0.0], prim_path:str=None, robot_name:str=None,recording_camera=None):
        # define world
        self.world = world
        ...
        self.rmpflow.set_robot_base_pose(self._initial_position, self._initial_orientation)
        self.articulation_rmpflow = ArticulationMotionPolicy(self._robot, self.rmpflow, default_physics_dt = 1 / 60.0)
    ...
    def check_gripper_arrive(self, target_position,stir=False) -> bool:
        ...
        current_position = (gripper_position + self.Rotation(gripper_orientation, torch.Tensor([0.0, 0.0, 0.05]))).cpu()
        error=F.mse_loss(current_position.clone().detach(),target_position.clone().detach()).item()
    ...
    def move_block_follow_gripper(self, attach_block, target_position, target_orientation=None):
        '''
        ...
        gripper_position, gripper_orientation = self.get_cur_grip_pos()
        # get block position
        block_position = gripper_position + self.Rotation(gripper_orientation, torch.Tensor([0.0, 0.0, 0.05]))
